---
Category: Entity
Sub-category: Infrastructure
Type: Hardware
Sub-type: Hypersonic Weapons System
Status: Active
Confidence: Verified
Developer:
  - Lockheed Martin
  - Dynetics
Platforms:
  - Dark Eagle (LRHW)
  - Conventional Prompt Strike (CPS)
Range: "3,500+ km (2,175+ miles)"
Speed: "Mach 5+ (3,800+ mph)"
Connected-to:
  - "[[Mondaloy]]"
  - "[[Monica Reza]]"
  - "[[William McCasland]]"
  - "[[Operation Epic Fury]]"
  - "[[Sandia National Laboratories]]"
Links:
  - "[[Mondaloy]]"
  - "[[Monica Reza]]"
  - "[[William McCasland]]"
  - "[[Operation Epic Fury]]"
  - "[[The Mondaloy Chain]]"
Tags:
  - infrastructure
  - hardware
  - hypersonic
  - boost-glide
  - conventional-deterrence
  - Dark-Eagle
  - Mondaloy
  - Operation-Epic-Fury
Aliases:
  - Common Hypersonic Glide Body
  - C-HGB
---
# Common Hypersonic Glide Body (C-HGB)

Standardized non-nuclear hypersonic glide vehicle developed jointly by the Army and Navy for conventional deep-strike capability. Developed by Lockheed Martin and Dynetics. The "common" design is shared across two platforms: the Army's Dark Eagle (Long-Range Hypersonic Weapon / LRHW) and the Navy's Conventional Prompt Strike (CPS) program.

## Technical Overview

The C-HGB detaches from a booster rocket at the edge of space, then glides through the upper atmosphere at Mach 5+. Unlike ballistic missiles which follow a predictable arc, the glide body performs high-G maneuvering during descent — making it extremely difficult for existing air defense systems to intercept. The weapon does not require a large explosive payload: kinetic energy at Mach 5+ is sufficient to collapse hardened underground bunkers.

**Range and speed:** 3,500+ km, Mach 5+. Launched from Guam, the weapon can strike mainland Chinese targets in under 20 minutes.

## Operational Status (2026)

**March 20, 2026:** U.S. Army officially declared the "hypersonic gap" closed with the first full Dark Eagle battery entering active service at Joint Base Lewis-McChord.

**USS Zumwalt:** The Navy's Zumwalt-class destroyers completed conversion in early 2026 — replacing the failed 155mm guns with large-diameter tubes specifically designed for C-HGB launch. The Zumwalt is the primary naval C-HGB platform.

**Unresolved testing concern:** A March 2026 DOT&E (Director, Operational Test and Evaluation) assessment noted "insufficient data" regarding C-HGB survivability in contested electromagnetic environments — meaning the weapon was fielded before testing was complete.

## May 2026 — Sonic Boom Clustering (Notation)

Late May 2026 saw an unusual cluster of unexplained atmospheric sonic boom events:

- **May 29 — South Carolina:** Unexplained sonic boom across the Midlands. USGS ruled out seismic activity. NASA ruled out launches and meteors. No official explanation provided.
- **May 30 — New England:** Sonic boom across NH/MA/RI. Subsequently attributed to a ~1-yard meteor entering the atmosphere off the coast of Boston, impacting Cape Cod Bay at ~100ft depth. Energy estimated at 300 tons TNT equivalent.

**The South Carolina event remains unexplained.** Standard CONUS test ranges (Pacific Missile Range at Kauai, Cape Canaveral) make C-HGB testing over South Carolina's interior unlikely by standard protocol. However, the LRHW battery at JBLM entered operational status the same month, and DOT&E noted insufficient operational testing data as recently as March 2026.

*This notation records a temporal correlation, not a confirmed connection. Filed as pattern of interest.*

## The Mondaloy Chain Connection

The C-HGB development chain runs directly through the cluster of deaths and disappearances documented in [[The Mondaloy Chain]]:

- **[[Monica Reza]]** — inventor of [[Mondaloy]], the nickel superalloy enabling C-HGB booster survivability at hypersonic velocities. Missing June 22, 2025.
- **[[William McCasland]]** — former AFRL Commander who funded the Mondaloy research program. The person who connected the materials science layer to the operational weapons layer. Missing February 27, 2026 — 24 hours before [[Operation Epic Fury]].
- **[[Sandia National Laboratories]]** — lead C-HGB design agent. [[DOGE]] fired 177 NNSA employees including Sandia personnel in February 2026 — the same month as the Iran strikes.

Whether C-HGB was used in Operation Epic Fury is not confirmed in open-source reporting. The timing of the pattern — materials scientist missing June 2025, weapons funding authority missing February 27 2026, Iran strikes February 28 2026 — is documented in the vault without asserting causation.

## Citations

- Army Dark Eagle fielding March 2026, battery at JBLM: [DefenseScoop January 2026](https://defensescoop.com/2026/01/21/dark-eagle-hypersonic-weapon-army-fielding-plans/)
- 20-minute China strike capability, DOT&E insufficient data: [19FortyFive March 2026](https://www.19fortyfive.com/2026/03/hypersonic-dark-eagle-missile-can-hit-china-or-russia-in-under-20-minutes-the-pentagon-just-admitted-it-hasnt-finished-testing-it/)
- USS Zumwalt CPS conversion: [Warrior Maven December 2025](https://warriormaven.com/news/sea/us-navy-zumwalt-destroyers-to-fire-hypersonic-cps-missiles-in-2026)
- South Carolina sonic boom May 29 2026: multiple news sources
- New England sonic boom/meteor May 30 2026: NASA, CBS Boston, Boston Globe
- Mondaloy materials chain: [[Mondaloy]] vault note | [[The Mondaloy Chain]] MOC
